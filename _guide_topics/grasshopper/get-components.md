---
title: Get Component
description: Get components are context sensative input and output components in Grasshopper
...
https://www.rhino3d.com/features/grasshopper/player/   this pops up as search hit #1, but I guess you want more detail?

I don't think we have documentation on the options. Here's what the "Get Number" options do
"Get Number" - the name of the component is used in the context of Hops to be the name used on an input parameter. In the context of gh player, the name is not used
"Prompt" - This is an input parameter tooltip when used in the context of Hops. This is the command line message in the context of the gh player
"At Least" - minimum number of inputs the component wants
"At Most" - the maximum number of inputs the component wants. If blank, there is no maximum. In the context of Hops if both AtLeast and AtMost are 1, then the input for the Hops component is set to item access (very useful). In the context of the gh player, it helps Rhino figure out when to stop asking for more numbers
"Minimum"/"Maximum" - limits that can be set for the component of allowable numbers. In the context of Hops, we can return an error message when values outside the allowable range are input. In the context of the gh player, these limits are enforced on the command line when asking for values.
